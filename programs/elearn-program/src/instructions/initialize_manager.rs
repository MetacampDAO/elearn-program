...
pub struct InitializeManager<'info> {
  ...
  #[account(
    init, 
    seeds = [MANAGER_PROOF_SEED, master.to_account_info().key.as_ref()],
    bump,
    payer = master,
...
pub fn handler(
  ...
  ctx.accounts.manager_proof.initialize(ctx.accounts.master.to_account_info().key(), manager_bump);
  let full_permission  = PermissionType::NORMAL
    | PermissionType::CERT
    | PermissionType::BATCH
    | PermissionType::ADMIN;
  ctx.accounts.manager_proof.set_type(full_permission);
  Ok(())
}