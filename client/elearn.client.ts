...
export class ElearnClient extends AccountUtils {
  ...
  async getCertficateAccVersion(certficatePDA: PublicKey) {
    ...
    const version = VersionLayout.decode(data, 8).version;
    return version;
  }
  ...
  async findManagerProofPDA(managerKey: PublicKey) {
    return await PublicKey.findProgramAddress(
      [
        Buffer.from(anchor.utils.bytes.utf8.encode("manager-seed")),
        managerKey.toBytes(),
      ],
  ...
  async initializeManager(
    ...
    const txSig = await this.elearnProgram.methods
      .initializeManager(managerBump)
      .accounts({
  ...
  async modifyManager(
    ...
    const adminPk = isKp(admin) ? (<Keypair>admin).publicKey : admin;
    const txSig = await this.elearnProgram.methods
      .modifyManager(targetPermissions)
      .accounts({
        admin: adminPk as any,
        adminProof,
        managerKey,
  ...
  async createBatch(
    ...
    const managerPk = (
      isKp(manager) ? (<Keypair>manager).publicKey : manager
    ) as PublicKey;
    const [batch, batchBump] = await this.findNewBatchPDA(managerPk);

    const txSig = await this.elearnProgram.methods
      .createBatch(batchName, batchBump)
      .accounts({
  ...
  async createCertificate(
    ...
    const [certificate, certificateBump] = await this.findNewCertificatePDA(
      batch
    );
